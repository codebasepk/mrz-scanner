...
//    private TextView translationView;
    ...
    public void onCreate(Bundle icicle) {
        ...
        toggleButton = findViewById(R.id.torch_button);
        toggleButton.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (cameraManager != null) {
                    if (b) {
                        cameraManager.doSetTorch(true);
                    } else {
                        cameraManager.doSetTorch(false);
                    }
                }
            }
        });
            ...
            if (ocrResult.getMeanConfidence() >= 70 && textResultTmpArr.length >= 2 && textResultTmpArr.length <= 3) {
                try {
                    ...
                    if (mrzInfo.toString().equals(result)) {
                        cameraManager.takePicture((ImageView) findViewById(R.id.image_view));
